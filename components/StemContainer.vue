...
const { storedValue: storedAnswers, setValue: updateStoredAnswers } =
  useLocalStorage("answerData", []);
...
<template>
  <!-- Main Header -->
  <div
    class="w-full h-14 bg-blue-400 dark:bg-black fixed z-40 top-0 flex justify-center items-center"
  >
  ...
  <div class="w-full min-h-screen flex flex-col items-center gap-4 py-20 px-2">
    <!-- Single Stem Panel -->
    <div
      class="flex flex-col gap-4 lg:flex-row lg:justify-around lg:w-[1000px]"
    >
      <!-- Stem Text -->
      <div
        class="p-5 w-full border-2 rounded-lg h-fit dark:text-white bg-white dark:bg-zinc-900"
      >
        <p class="text-lg font-bold">STEM 1</p>
        <hr class="my-2" />
        <ScrollPanel
          class="h-56 lg:w-[300px] lg:h-80 lg:max-h-80"
          style="width: 100%"
        >
          <div class="w-full pr-2 gap-4 flex flex-col lg:mr-0 pb-10">
            <p>
              Baterai menggunakan energi dari reaksi kimia untuk memisahkan
              muatan negatif dan positif di dalam baterai. Proses ini
              menghasilkan peningkatan energi potensial elektrostatik (muatan
              listrik diam) dari muatan yang mengarah ke perbedaan tegangan.
              Baterai senter yang biasa ditemui memiliki beda potensial 1,5 volt
              di antara kedua terminal (kutub posiif dan negatif baterai).
            </p>
            <p>
              Karena ada kelebihan muatan positif di salah satu terminal baterai
              dan kelebihan muatan negatif di terminal baterai yang lain, muatan
              ini akan cenderung bergabung kembali dengan mengalir dari satu
              terminal ke terminal yang lain jika terdapat penghantar antara
              kedua terminal. Muatan-muatan ini hanya dapat mengalir melalui
              penghantar eksternal. Namun karena adanya gaya yang berlawanan
              terkait dengan reaksi kimia dalam baterai, muatan akan mengalir
              melalui penghubung ke terminal yang berlawanan.
            </p>
            <p>
              Satuan internasional (SI) untuk arus listrik adalah Ampere (A),
              yang didefinisikan sebagai 1 coulomb per detik, sedangkan coulomb
              dapat dimaknai sebagai banyaknya muatan. Satuan ampere diambil
              dari nama penemunya yang seorang ahli mmatematika dan fisika
              bernama Prancis Andre Marie Ampere tahung 1775-1836.
            </p>
            <PreviewableImage
        ...
        <Accordion :activeIndex="null">
          ...
          >
            ...
            <div class="py-5 flex flex-col gap-2">
              ...
              >
                <RadioButton
                  :inputId="`option-${question.id}-${i}`"
                  :name="'question-' + question.id"
                  :value="i"
                  v-model="question.answer"
                />
                <label :for="`option-${question.id}-${i}`" class="ml-2">{{
                  option
                }}</label>
              </div>
            </div>
            <div
              ...
              @click="
                () => {
                  question.notSure = !question.notSure;
                  submitAnswers();
                }
              "
            >
    ...
    <!-- Navigation Buttons -->
    <div
      class="w-full h-14 bg-blue-400 dark:bg-black fixed z-10 bottom-0 flex justify-center items-center gap-4"
    >
      <Button
        :severity="'info'"
