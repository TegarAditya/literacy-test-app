            ...
            >
              ...
              <div class="border-2 rounded-lg overflow-hidden w-full h-fit">
                <Accordion :activeIndex="null">
                  <AccordionTab
                    v-for="(question, questionIndex) in stem.attributes
                      .questions.data"
                    :key="question.id"
                  ...
                  >
                    ...
                    <div
                      class="m-0"
                      v-html="question.attributes.question_field"
                    ></div>
                      ...
                      <div
                        ...
                        @click="
                          getAnswerForQuestion(
                            question.attributes.code
                          ).option_code = option.attributes.code
                        "
                      >
                        <RadioButton
...
onMounted(() => {
  ...
  setInterval(() => {
    if (activeQuestion.value) {
      const activeAnswer = getAnswerForQuestion(
        activeQuestion.value.attributes.code
      );
      if (activeAnswer) {
...
function getAnswerForQuestion(questionCode: string): Answer {
  return (
    answers.value.data.find(
      (answer) => answer.question_code === questionCode
    ) || { question_code: "", option_code: null, isNotSure: false }
  );
}
